...
from lattice_data_tools.gm2.HVP.bounding_methods import MesonPair
        ...
        for fermion_type in fermion_types:
            ...
            for corr in ["sim", "pp"]:
                ...
                for window in windows:
                    print("   Window:", window)
                    lambda_bts = lambda i: [MesonPair(M_meson=aMpi_phys[i], L=L, window=window, a_fm=a_fm[i], V=VkVk_charge[i,:], K=K[i,:], Z_ren=Z_ren, t0=t0, strategy="trapezoidal") for t0 in ti]
                    a_mu_eff = BootstrapSamples.from_lambda(N_bts=N_bts, fun=lambda_bts, parallel=True)
                    a_mu_2pions_dict[ens_name][fermion_type][corr][f"{window}_window"] = a_mu_eff
    #---------------
    with_dill.dump(a_mu_2pions_dict, f'{fpi_dir}/a_mu-2pions.pkl') # Save to pickle
#---
