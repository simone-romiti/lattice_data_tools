...
4. **In a typical analysis, how should one apply a function/combine different observables and estimate the error?**
      _Example:_ We measure the pion and kaon mass from lattice data and we want to give a prediction of the continuum limit of their ratio.
    - _Note:_ In general, when modifying different observables (by applying functions, combining them, etc) the distribution changes.
    - With bootstraps however, it is sufficient to apply the functions/combine observables by doing it for each average (i.e. bootstrap-by-bootstrap). If the function is differentiable, the final result can be treated as Gaussian: for asymptotically Gaussian variables the distribution remains asymptotically Gaussian (see Delta Method: https://en.wikipedia.org/wiki/Delta_method)
    - Thus we can estimate the "error" on the mean (i.e. statistical uncertainity) on our composite observables by treating them as Gaussian.
...
print("Bootstrap:", x_bts.mean(), "|", np.sqrt(N)*x_bts.error())
```
...
# < (y-<y>)^2 > = <y^2> - (<y>)^2
dy2 = y2_bts - (y_bts.mean())**2

print("Variance")
print("Exact: ", 1/lambda_param**2)
print("Bootstrap:", dy2.mean())
```
...
print("Bootstrap:", x_bts.mean(), "|", x_bts.mean())
```
...
plt.hist(sinx2_bts[1:], bins=int(np.sqrt(N)))
plt.show()
plt.close()
...
print("Bootstrap:                  ", sinx2_bts.error())
print("Bootstrap (WRONG APPROACH): ", sinx2_bts_WRONG.error())
```