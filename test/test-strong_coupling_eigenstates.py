"""
Generating samples according to the |\\psi|^2 of a strong coupling eigenstate and measuring the energy.
"""
...
psi_SC = lambda U: suN.get_Tr(U.plaquette(x=x0, mu=0,nu=1))
...
psi_n = lambda U: suN.get_Tr(GaugeConfiguration(U).plaquette(x=x0, mu=0,nu=1))
def omeas(i, Ui):
    CM = CanonicalMomenta(U=GaugeConfiguration(Ui))
    res = [
        Ui,
...
n_therm = 1000
U_langevin = GaugeConfiguration(torch.flatten(torch.stack([omeas_i[0] for omeas_i in LG_evol["Oi"]], dim=0)[n_therm:,...], start_dim=0, end_dim=1))
...
La_psi_tensor = torch.stack([omeas_i[3] for omeas_i in LG_evol["Oi"]], dim=0)[n_therm:,...]

print(La_psi_tensor.shape, "La_psi shape")
n_axes = len(La_psi_tensor.shape)
La_psi_abs2 = (La_psi_tensor.abs()**2).sum(dim=tuple([i for i in range(1,n_axes)]))

print(La_psi_abs2.shape)

psi_SC_abs2 = psi_SC_tensor.abs()**2
r1 = La_psi_abs2/psi_SC_abs2
r2 = psi_n_tensor.abs()**2/psi_SC_abs2
La2_est = (r1+ 0.5*r2).mean()

print(La2_est.item(), r1.mean().item(), r2.mean().item())

# print(psi_SC_tensor.shape)
# La2_psi_over_psi = Oi_tensor # torch.einsum("i...,i->i...", Oi_tensor, 1/psi_SC_tensor.squeeze())
# N_tot = La2_psi_over_psi.shape[0]
# La2_psi_mean = torch.tensor([(La2_psi_over_psi[i,...].abs()**2).sum(dim=-1).mean(dim=0).sum()/Z - psi_n_tensor[i,...].abs()**2 for i in range(N_tot)])
...
# print("Bootstrap Numerical integral (no Z):", 1+La2_psi_bts.mean(), La2_psi_bts.error())
...
CM = CanonicalMomenta(U=GaugeConfiguration(U_langevin))
print(U_langevin.shape)
#print(CM.La_chain_rule(f=Laf, U=U_langevin))

# LD = LieDerivatives(U=U0)
# t1 = time.time()
# CM2 = LD.La_squared_per_link(f=psi_SC, U=U0).detach()
# t2 = time.time()
# print("dt autodiff:" , t2 - t1)
# print("With autodiff (U0):", U0.n_links * CM2/psi_SC(U0))
