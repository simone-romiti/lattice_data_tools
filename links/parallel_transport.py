...
#     ParallelTransporters = torch.zeros(*new_shape, dtype=U.dtype, device=U.device)    # setting identity element for k=0 --> no transport
#     U0 = U.clone()
...
def get_ParallelTransporters(U: GaugeConfiguration, K: int):
    ...
    assert all(K <= Lmu for Lmu in lattice_shape)
    d = U.n_dims

    # Identity for k=0 slot: use a cold-start copy
    U0 = suN.get_coldstart(shape=U.shape, dtype=U.dtype, device=U.device, requires_grad=U.requires_grad)

    # Build slices per mu, collect into a list, then stack — no in-place writes.
    per_mu = []
    for mu in range(d):
        Lmu = U.shape[1 + mu]

        # k=0 slot: identity (cold-start)
        slots = [None] * (2 * K + 1)
        slots[K] = U0[..., mu, :, :]  # shape: (batch, L1,...,Ld, Nc, Nc)

        # Build backward transporters: slots K-1, K-2, ..., 0
        for k in range(1, K + 1):
            i_bkw = K - k
            idx_bkw = (torch.arange(Lmu, device=U.device) - k + Lmu) % Lmu
            U_bkw = U.index_select(1 + mu, idx_bkw)[..., mu, :, :]
            slots[i_bkw] = slots[i_bkw + 1] @ U_bkw.adjoint()

        # Build forward transporters: slots K+1, K+2, ..., 2K
        for k in range(1, K + 1):
            i_fwd = K + k
            idx_fwd = (torch.arange(Lmu, device=U.device) + k - 1) % Lmu
            U_fwd = U.index_select(1 + mu, idx_fwd)[..., mu, :, :]
            slots[i_fwd] = slots[i_fwd - 1] @ U_fwd

        per_mu.append(torch.stack(slots, dim=-3))  # (batch, L1,...,Ld, 2K+1, Nc, Nc)

    # Stack over mu dimension
    U_PT = torch.stack(per_mu, dim=-4)  # (batch, L1,...,Ld, d, 2K+1, Nc, Nc)
