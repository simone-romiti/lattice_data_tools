...
class CanonicalMomenta:
    ...
    def La_chain_rule(self, f: typing.Callable, U: GaugeConfiguration):
        ...
        """
        delta = torch.zeros(size=U.shape, dtype=U.dtype, device=U.device, requires_grad=True) # Note: delta should be complex
        f_U = f(GaugeConfiguration(U + delta)) + 0.0*1j # f(U+delta)
    ...
    def LaLa_chain_rule_EXPERIMENTAL(self, f: typing.Callable, U: GaugeConfiguration):
        ...
        dRef_dU = my_autograd(y=f_U.real, x=delta1, grad_outputs=torch.ones_like(f_U.real), create_graph=True, retain_graph=True).unsqueeze(-3)
        dImf_dU = my_autograd(y=f_U.imag, x=delta1, grad_outputs=torch.ones_like(f_U.imag), create_graph=True, retain_graph=True).unsqueeze(-3)
        df_domega_ab = chain_rule_contributions(A=A, dRef_dU=dRef_dU, dImf_dU=dImf_dU).sum(dim=(-2,-1)) # summing over the color components
        Laf_ab = -1j * df_domega_ab
        # print(dRef_dU.shape, Laf_ab.shape, delta.shape)
        cosd_Laf = torch.einsum("...ij,...aij->...aij", torch.cos(delta.abs()), Laf_ab)
        dReLaf_dU = my_autograd(y=cosd_Laf.real, x=delta2, grad_outputs=torch.ones_like(cosd_Laf.real), create_graph=True, retain_graph=True).unsqueeze(-3)
        dImLaf_dU = my_autograd(y=cosd_Laf.imag, x=delta2, grad_outputs=torch.ones_like(cosd_Laf.imag), create_graph=True, retain_graph=True).unsqueeze(-3)
