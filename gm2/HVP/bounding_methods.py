...
from lattice_data_tools.gm2.HVP.windows import a_mu
...
def ZeroTail(window : Literal["SD", "W", "LD", "full"], a_fm: float, V: np.ndarray, K: np.ndarray, Z_ren: float, t0: int, strategy="trapezoidal") -> float:
    ...
    return a_mu.with_precomputed_K(window=window, a_fm=a_fm, t_lat=ti[1:t0], Vi=V[1:t0], K=K[1:t0], Z_ren=Z_ren, strategy=strategy)
####
...
def MesonPair(M_meson: float, L: float, window: Literal["SD", "W", "LD", "full"], a_fm: float, V: np.ndarray, K: np.ndarray, Z_ren: float, t0: int, strategy="trapezoidal") -> float:
    ...
    T_ext = V.shape[0]
    ti = np.arange(0, T_ext)
    res1 = a_mu.with_precomputed_K(window=window, a_fm=a_fm, t_lat=ti[1:t0], Vi=V[1:t0], K=K[1:t0], Z_ren=Z_ren, strategy=strategy)
    ...
    res2 = a_mu.with_precomputed_K(window=window, a_fm=a_fm, t_lat=ti[t0:T_ext], Vi=V_2pi_tail, K=K[t0:T_ext], Z_ren=Z_ren, strategy=strategy)
    res = (res1+res2)
    return res
####
...
def M_eff_t0_Tail(M_eff: np.ndarray, window: Literal["SD", "W", "LD", "full"], a_fm: float, V: np.ndarray, K: np.ndarray, Z_ren: float, t0: int, strategy="trapezoidal") -> float:
    ...
    res = 0.0
    if t0 > 0:
        res = a_mu.with_precomputed_K(window=window, a_fm=a_fm, t_lat=ti[1:t0], Vi=V[1:t0], K=K[1:t0], Z_ren=Z_ren, strategy=strategy)
        if t0 < T_ext-1:
...
class FiniteVolume:
    ...
    def Gounaris_Sakurai(
        ...
        T_ext = V_lat.shape[0]
        ti = np.arange(0, T_ext)
        a_mu_left = a_mu.with_precomputed_K(window=window, a_fm=a_fm, t_lat=ti[0:t0], Vi=V_lat[0:t0], K=K[0:t0], Z_ren=Z_ren, strategy=strategy)
        a_mu_right = a_mu.with_precomputed_K(window=window, a_fm=a_fm, t_lat=ti[t0:T_ext], Vi=V_PP[t0:T_ext], K=K[t0:T_ext], Z_ren=Z_ren, strategy=strategy)
