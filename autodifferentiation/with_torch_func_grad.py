...
class LaplacianGenerator_new:
    """
    Class that generates a (potentially compiled) version or a vectorized calculator of the contributions to the Laplacian of a function, i.e.
    $$\\partial_i f(x1,...,xn)$$
    """
    def __init__(self, f: typing.Callable, x: torch.tensor, *args, do_compile: bool):
        """
        f: function taking as arguments a tensor of shape (n_variables,)
        x: example input of f(x) for each batch: shape==(batchsize,n_variables).
           NOTE:
        ...
        self._input_shape = x.shape # saving the expected input shape
        n_var = x.shape[-1] # number of variables for f(x)
        device = x.device # same device as the input
        n_args = len(args) # number of non-differentiated arguments 

        def f_shift(xb, delta, i, *args_i):
            n = xb.shape[-1]
            ei = (torch.arange(n, device=xb.device) == i).to(xb.dtype)
            return f(xb + delta * ei, *args_i)
        ...
        d2f_vmapped = torch.func.vmap(
            torch.func.vmap(
                d2f,
                in_dims=(None, None, 0) + (None,) * n_args # parallelizing only over the variable index --> \\partial_{x_i}^2 f(x)
           ),
            in_dims=(0, None, None) + (None,) * n_args,  # parallelizing only over the batch index f(x1^{i}, x2^{i},...)
        )

        delta = torch.tensor(0.0, device=device)
        indices = torch.arange(n_var, device=device)
        uncompiled_d2f = lambda x, *args: d2f_vmapped(x, delta, indices, *args)
        if do_compile:
            self._d2f_function = get_compiled_function(uncompiled_d2f, x, *args)
        else:
            self._d2f_function = uncompiled_d2f
