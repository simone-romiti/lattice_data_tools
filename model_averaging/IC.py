...
def get_IC(
        ch2: np.ndarray, n_par: np.ndarray, n_data: np.ndarray, 
        IC: valid_IC, 
    ...
    if IC == "AIC":
        ...
        raise ValueError(f"Unknown information criterion IC='{IC}'.")
...
def get_weights(ch2: np.ndarray, n_par: np.ndarray, n_data: np.ndarray, IC: valid_IC, Nmax: Optional[int] = None) -> np.ndarray:
    ...
    """
    A = get_IC(ch2=ch2, n_par=n_par, n_data=n_data, IC=IC, Nmax=Nmax)
    w_i = np.exp(-A/2)
    return w_i
#---
